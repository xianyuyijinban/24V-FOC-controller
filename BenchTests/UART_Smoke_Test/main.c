...
static const uint8_t s_uartSmokeBanner[] = "UART_SMOKE SWAP HSE OK\r\n";

static void UART_Smoke_BusyDelay(volatile uint32_t cycles);
static void UART_Smoke_SuccessLoop(void);
static void UART_Smoke_FailureLoop(void);
...
static void UART_Smoke_SuccessLoop(void)
{
    ...
    while (1) {
        ...
        for (burst = 0U; burst < 40U; ++burst) {
            (void)HAL_UART_Transmit(&huart1,
                                    (uint8_t *)s_uartSmokeBanner,
                                    (uint16_t)(sizeof(s_uartSmokeBanner) - 1U),
                                    100U);
        }
