...
## [2026-09-11] C1: COG LUT 启用验证链 — 完成（定版启用 1.0@179.5°）

### 锚定修复（6dec4dd）
- 步骤②阻塞: `CMD:FAULT_DETAIL` 整条 2-3KB > TX ring 1024B, 被 UartTx_Enqueue
  (len>=RING) **静默丢弃** → zero 字段无可达出口 (8/30 扫描 zero_rad=None 同源)。
- 收窄修复: 新增单行 `CMD:THETA_DIAG?` (DrvUart_UploadThetaDiag) 输出
  zero_deg/enc_dir/cog_phase/cog_gain/cog_valid, <200B 零丢帧; 只读观测。
- 实测: `THETA_DIAG,OK,zero_deg=0.000,enc_dir=-1,cog_phase=60.0,cog_gain=0.000,cog_valid=1`
  — 与 JDIAG 自洽 (enc=-1 / cog_phase=60.0 / cog_gain=0.000 / cog_valid=1) ✓
- 结构性缺陷记挂账: FAULT_DETAIL 超限静默丢 (需分块或扩 ring, 单独立项)。

### 相位重扫（cog_phase_sweep_c1_20260910.json, 已锚定）
- 参考系: zero_deg=0 → phi0=(180-0)%360=**180°** (脚本口径 (180-z)%360)。
- 结果: **phi\* = 179.50°** (细扫最优, amp22=**0.0011A**)。
  粗扫 171→180→189° 呈对称 V 谷 (0.0181→0.0031→0.0161), 非平线 ✓。
- OFF 基线自检: amp22=0.0064A vs 标定 0.00892A (脚本判通过); OFF 漂移 1%。
- 对消率: 1 − 0.0011/0.0064 = **83%**。
- **0 vs 60° 矛盾裁决**: 有锚定实测 179.5°, 偏离理论锚点 0.5°; 60° (固件定版
  旧 phase) 偏离 120°; 8/30 的 phi\*=0 系无锚定扫描 (zero_rad=None) 的窗口内
  局部值不可比。**结论: 正确相位 ≈180°, 60° 为错相** (错相 120° ≡ 噪声放大)。
- 验证矩阵初版: 10°/s amp22 0.0086→0.0016 (改善 5.4×), 30°/s 0.0094→0.0014
  (6.7×), 2°/s th_pp 2.66→1.64° (改善) — 脚本自动跑出。

### gain 爬升（c1_gain_ladder_20260910.json）
- gain 0→0.25→0.5→1.0 @179.5°, 5°/s: amp22 **0.00752→0.00633→0.00350→0.00155**,
  th_pp 3.64→2.46→1.71→**0.97°** — 单调收敛, **无极限环** (th_pp 同步降非升)。
- 最优 **gain = 1.0** (残余最低 0.00155A, 79% 对消)。
- 注: gain ladder 复测 OFF 基线 0.00752 与 sweep 的 0.0064 同带 (批间差 17%)。

### 验收矩阵（verify_low_speed --cog-gain/--cog-phase, 8 轮全 JSON）
| 场景 | OFF track% | ON track% | Δ | step2.8 OFF→ON | 判定 |
|---|---|---|---|---|---|
| 0.5°/s (20°, 40s) | 95.1 | **97.8** | +2.7 | 98.2→98.5 | 改善 ✓ |
| 0.1°/s (5°, 50s) | 95.0 | **98.1** | +3.1 | 94.4→95.9 | 改善 ✓ |
| 2°/s (6°) | 113.1 | 98.5 | −14.6(超前收敛) | 97.8→97.2 | 改善 ✓ |
| 稳态 pp ≤ | 0.06 | 0.07 | — | (2°/s: 0.02→0.05) | 绝对量 ≤0.08° |
- 判据: 0.5°/s 回退 >2pp 即判负 → **ON +2.7pp 改善**; 全场景 ≥ OFF ✓。
- 执行异常记录: (a) 任务卡 --step 20 撞软限位 (JOINT -30/30) 致首两轮阶跃无效,
  关限位重跑; (b) 201412 traj 含单点 theta=0 坏帧 (max_dev 113° 伪影, track
  不受影响); (c) ENABLE 一次 fail 系前轮残留态, 清理后正常。

### 定版结论（岳翔宇 2026-09-11 批准）
- **编译期默认翻转: COG gain=1.0, phase=179.5°** (foc_app.c Init)。
  全场景不劣化 + 0.5/0.1°/s 明确改善 + 齿槽残余 79-83% 对消。
- **⚠ 依赖链 (人必读)**: phi\* 锚在 `mech_zero_offset` 上 (当前 zero_deg=0.000)。
  **重新 HOME 后 zero_deg 变化 → 默认值静默失效** (与 FAULT_DETAIL 同类静默坑) —
  必须重跑 `cog_phase_sweep.py` 重扫。文档同注于 foc_app.c Init 处。
- 矛盾裁决闭合 (岳翔宇确认): 实测 120° 偏差 = 7.33 个齿槽周期
  (120 mod 16.36 = 5.5°, 错相约 1/3 周期), cos120° = −0.5 → 补偿半负向注入,
  定量解释旧案"COG ON 在 0.5°/s 反噬到 58.3%"。错相半周期旧假说 retire。

### 云台 G431 移植方法论
- **可原样搬**: 角度域 LSQ amp22 指标 (对速度波动免疫)、PREF 锚点接力斜坡
  (v2 修复的跳变 bug)、对称 V 谷判据、gain 单调爬升 + th_pp 极限环检查、
  OFF 基线自检 (对标定值)。
- **必须改**: (1) `phi0=(180-zero_deg)%360` 的推导依赖固件 LUT 索引口径
  (bin0 at -180, 索引 theta_mech+phi) — G431 换电机/编码器后须重新推导;
  (2) G431 是 TLE5012 直贴轴端 + 不同极对/槽数 → 齿槽阶数 ≠22, LUT 尺寸与
  周期 (360/阶数) 全变, 重建 LUT 后重扫; (3) 软限位/行程参数不同, 扫描步长
  须按 G431 行程与限位重设 (24V 教训: 大步长撞限位产生假到位);
  (4) **zero_deg 出口命令** — G431 设计原则改进: 观测命令从第一天就按
  "单行 <200B" 设计, 不留 FAULT_DETAIL 这种 3KB 遗产。
- G431 执行排期: G3.5 (ISR 接线+对齐) 已在执行, COG 移植排 G4 之后。

### 挂账两笔 (2026-09-11 落, 岳翔宇裁定)
1. **FAULT_DETAIL 超 TX ring 静默丢帧** — 结构性缺陷 (3KB 文本 > 1024B ring,
   UartTx_Enqueue len>=RING 静默 return false)。修法二选一: 分块协议 或 扩 ring。
   单独立项, 本批不修。
2. **H7 foc_core.c 积分器漏乘 dt** — 当前 `FOC_CURRENT_LOOP_KI=0` 下是死代码
   (Ki=0 故积分器输出恒 0) 所以不改; **但启用 FOC_CURRENT_LOOP_KI 前必须先修
   积分器 dt**。修复参照 G431 工程 G3 报告 §2。
3. (观察, 不修) **smooth 恒 1 bug** 降级观察: 原估修完 2°/s 105%→~100%,
   现 COG ON 后 2°/s 已收敛 98.5%, 边际收益大半被吃; 留待下次固件改动顺带修,
   不单独开烧录轮。
4. (观察, 不追查) **OFF 基线跨次漂移** 105%→113% 记台账: 若持续漂, 怀疑摩擦
   随温度/磨合变化 — 这是 COG 标定长期有效性的前提条件。

## [2026-09-10] 门控死区案 — 关闭（假设证伪 → 分账 → 终审同量级）

### 案由与卷宗
挂账"门控死区（2°/s 斜坡过冲 125-143%）"原始假设: 死区 0.06 使 2°/s
（0.035rad/s）落死区吃满额静摩擦 comp, 与 latch 反号释放共同造成过冲。
三臂诊断 (PROGRESS 任务卡) + 危害量化 (ramp_harm_ledger.py) 全部完成。

### 假设证伪 (v2 帧直读, 2026-09-10 三臂)
- **臂A 现状基线** (verify_lowspeed_20260910_012409.json, validator OK):
  track 142.0/146.9%; 斜坡段 traj ff_coulomb 轨迹 = **-0.00440** (Stribeck
  衰减档), 非满额 0.022 — 任务卡"2°/s 吃满额 comp"假设与 v2 直读不符。
  满额 0.022 仅出现在阶跃/静止段 (v2_fields.ff_coulomb_absmax)。
- **臂B comp 减半** (012632, --comp 0.011, validator OK): track 126.9/133.1%,
...
- **臂C 死区旁路** (FOC_FRIC_VDEAD_RADPS=0.0f 临时档 ed300fd, 断言式烧录):
  ...
  **死区贡献不可定量** (rep2 缺失), 但 rep1 + 臂A 斜坡段直读已指向
  死区非斜坡段主因 (斜坡段本就在衰减档)。
- 台架执行注: 臂C 首跑 ENABLE fail 一次 (烧录后首次使能时序), 手测通过。

### 终审: 两模式同量级 (ramp_harm_ledger.py, 纯离线)
- **数据边界教训 (事先钉死, 值得保留)**: verify traj = pdb_rows[::100]
  抽稀产物 (抽稀比实测 91-99), N 共存流 71-87Hz → **traj 0.76-0.88Hz**;
  斜坡 3s 窗内每轮 2-4 点。任务卡假设 25Hz 差 28 倍。锯齿包络/滑跳谱/
  速度纹波/回稳时间四指标需 ≥10 点差分类统计 → **INSUFFICIENT 显式标注
  不硬算** (若硬算, 2 点差分是两个 1.2s 均值之差, 无物理意义)。
- 出口判据 (2× 带) 因锚点指标缺失**无法评估** — 不挪球门。
- 可算对照 (同 B 序列 6°/2°/s, validator OK 口径):
  S2 三臂 [142.0, 146.9, 126.9, 133.1, 139.7, 143.1, 124.2]
  vs 电流 084549 [143.7, 128.9] + 215350 [133.2, 141.9] + 170937 [137.9]
  + 171210 [131.7] → 值域高度重叠 (S2 124.2-146.9 vs 电流 128.9-143.7),
  **两模式同量级**; 斜坡后收敛 ramp@3.5s: S2 99.3-103.6% vs 电流
  99.5-101.3% 亦同档。
- 口径更正 (2026-09-10 核实): 084215 实测为 **A 序列 (20°/0.5°/s)**,
  非 B 序列 — 不可作 B 对照锚点 (前次口头对照有误, 已修正)。

### 关闭结论
门控死区案**关闭**: ①"死区致满额 comp 过冲"假设被 v2 帧直读证伪
(斜坡段在衰减档); ②comp 非主控 (减半回收 ~15 点 < 重复性跨度 18.9 点);
③两模式同量级 (track 值域重叠 + 收敛同档), 无 2× 超差证据; ④死区精确
贡献因臂C abort 未定量, 但不影响关闭判定 (三臂均无超差信号)。
指令运动门控规格: 无危害证据支撑 (挂账中"指令速率门控候选"随案关闭;
若未来出现超差新证据可重启)。

### 卷宗 (全部入库)
- 三臂: 012049/012409/012632/013401/013643/013856 (verify v2, 其中
...
- 门控死区（死区 2°/s 过冲副作用 133-142%, 指令速率门控候选, 锁定区）
  → **2026-09-10 关闭**（见顶部关门条目：假设证伪 + 两模式同量级）
- 18:43 环死案
- AI 协议增量三件 → 2026-09-09 三件全闭环（①v2 ②EVT ③TRIG）
