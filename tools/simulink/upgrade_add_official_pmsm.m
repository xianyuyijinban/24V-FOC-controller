...
create_or_replace_constant(mdl, 'Ki_pos',   'Ki_pos',   [40 110 120 130]);
...
add_block('simulink/Sources/In1', [sys '/Ki_pos'], ...
    'Position', [30 175 60 189], 'Port', '4');

add_block('simulink/Math Operations/Sum', [sys '/pos_error'], ...
    'Inputs', '+-', ...
    'Position', [105 55 125 105]);
add_block('simulink/Math Operations/Product', [sys '/p_term'], ...
    'Inputs', '2', ...
    'Position', [175 45 215 75]);
add_block('simulink/Math Operations/Product', [sys '/i_input'], ...
    'Inputs', '2', ...
    'Position', [175 145 215 175]);
add_block('simulink/Discrete/Discrete-Time Integrator', [sys '/i_state'], ...
    'SampleTime', 'Ts_position', ...
    'InitialCondition', '0', ...
    'Position', [260 142 320 178]);
add_block('simulink/Math Operations/Sum', [sys '/pi_sum'], ...
    'Inputs', '++', ...
    'Position', [365 85 385 125]);
add_block('simulink/Discontinuities/Saturation', [sys '/speed_sat'], ...
    'UpperLimit', 'speed_limit', ...
    'LowerLimit', '-speed_limit', ...
    'Position', [435 85 485 125]);
add_block('simulink/Sinks/Out1', [sys '/speed_ref'], ...
    'Position', [540 98 570 112], 'Port', '1');

add_line(sys, 'pos_ref/1',       'pos_error/1', 'autorouting', 'on');
add_line(sys, 'theta_mech_fb/1', 'pos_error/2', 'autorouting', 'on');
add_line(sys, 'pos_error/1',     'p_term/1',    'autorouting', 'on');
add_line(sys, 'Kp_pos/1',        'p_term/2',    'autorouting', 'on');
add_line(sys, 'pos_error/1',     'i_input/1',   'autorouting', 'on');
add_line(sys, 'Ki_pos/1',        'i_input/2',   'autorouting', 'on');
add_line(sys, 'i_input/1',       'i_state/1',   'autorouting', 'on');
add_line(sys, 'p_term/1',        'pi_sum/1',    'autorouting', 'on');
add_line(sys, 'i_state/1',       'pi_sum/2',    'autorouting', 'on');
add_line(sys, 'pi_sum/1',        'speed_sat/1', 'autorouting', 'on');
...
add_line(sys, 'speed_mech_fb/1',  'speed_error/2', 'autorouting', 'on');
add_line(sys, 'speed_error/1',    'p_term/1',      'autorouting', 'on');
add_line(sys, 'Kp_speed/1',       'p_term/2',      'autorouting', 'on');
add_line(sys, 'speed_error/1',    'i_input/1',     'autorouting', 'on');
add_line(sys, 'Ki_speed/1',       'i_input/2',     'autorouting', 'on');
add_line(sys, 'i_input/1',        'i_state/1',     'autorouting', 'on');
add_line(sys, 'p_term/1',         'pi_sum/1',      'autorouting', 'on');
add_line(sys, 'i_state/1',        'pi_sum/2',      'autorouting', 'on');
...
add_line(mdl, 'Kp_pos/1',                  'position loop/3', 'autorouting', 'on');
add_line(mdl, 'Ki_pos/1',                  'position loop/4', 'autorouting', 'on');

add_line(mdl, 'position loop/1',           'speed loop/1', 'autorouting', 'on');
add_line(mdl, 'feedback/4',                'speed loop/2', 'autorouting', 'on');
...
clear_block_ports([mdl '/position loop'], 'Inport', 1:4);
...
set_block_position_if_exists([mdl '/Ki_pos'],             [40 110 120 130]);
