...
### 定版结论
- **COG LUT 启用: `CMD:COG_CFG,1.0,179.5`** (gain 1.0, phi 179.5°)。
  全场景不劣化 + 0.5/0.1°/s 明确改善 + 齿槽残余 79-83% 对消。
- 依赖: phi\* 依赖 mech_zero_offset — **重新 HOME 后须重扫**。
- 固件默认值改动待岳翔宇确认 (当前默认仍 0.0/60.0 编译值, 运行时设)。
...
- **必须改**: (1) `phi0=(180-zero_deg)%360` 的推导依赖固件 LUT 索引口径
  ...
  (4) zero_deg 出口: G431 若无等价短命令, 须先补 (FAULT_DETAIL 超限坑同源)。
