...
class TestBuildSystemConsistency(unittest.TestCase):
    ...
    def test_speed_feedback_and_position_steps_are_bench_safe(self):
        ...
        # 慢摇连续静摩擦补偿: 指令方向锁存替代误差阈值(3°/1°)
        self.assertIn("float ref_delta = handle->pos_ref - handle->pos_ref_prev;", foc_c)
        self.assertIn("handle->pos_cmd_dir = (ref_delta > 0.0f) ? 1.0f : -1.0f;", foc_c)
        self.assertIn("if (handle->pos_cmd_dir > 0.0f)", foc_c)
        self.assertIn("friction_dir = 1.0f;", foc_c)
        self.assertNotIn("friction_dir = pos_error_for_friction;", foc_c)
        self.assertNotIn("handle->position_friction_active = 1U;", foc_c)
        # 直连位置环低速条件积分(消除静摩擦稳态误差)
        self.assertIn("handle->pos_integral += pos_error * FOC_POS_LOOP_TS;", foc_c)
        self.assertIn("FOC_POS_INTEGRAL_ERR_RAD", foc_c)
