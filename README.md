...
项目包含 STM32H743 固件、PySide6 上位机、高速电流遥测，以及把电机变成带力反馈鼠标滚轮的 Windows Bridge。当前经过台架验证的标准工作平台是 **24V**（2026-09-16 起，阈值 14V/28V 已烧入并台架验收）；`FW_INFO` 中的 `baseline=12V_STANDARD` 为历史命名，实测母线为 24V。
...
1. 检查三相线、编码器、DRV8350S、电流采样和 24V 电源接线。
2. 编译并烧录固件。
3. 使用 HostComputer 以 `1000000 baud` 连接。
4. 查询 `SYS:FW_INFO?`，确认 `baseline=12V_STANDARD`（历史命名，实际为 24V 平台）。
5. 新电机先设置极对数并执行参数识别。
6. 从较小电流和较低速度开始验证方向。
...
- 当前正式基线供电为 24V（欠压/过压阈值 14V/28V，2026-09-16 台架验收）；12V 供电需改回旧阈值固件，不再维护。
