...
- **G4** verify_low_speed PASS (判据): 稳态 pp=0.04°, 斜坡 95.5%, 阶跃 88.3%。
  ...
  **数据对照 (阶跃 2.0s 到位%)**: 08-30 11:24 = 97.0%, 08-30 17:15 = 97.8%, **今天 88.3%** —
  同判据下 9+ 个百分点真实回退, 非判据差。候选变量: 今天 run 前 fault=7 (ADC 采样偶发,
  第一轮 pp=6.31°) CLEAR_FAULT 后余波 / 探针+门控固件 vs 0fa8b17。机制未确认 —
  下次台架干净系统复跑核对 (登记为尾差复跑项, 不写"已查明")。
