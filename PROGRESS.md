...
### Problem / Task
- 手感"旋转微抖"开出高速纹波战线；0.5°/s 定版（08-20）后暴露的全速段平滑性问题。
- 最终收敛到两条独立根因链：
  1. **C 相电流采样模拟链路缩水 41%**（channel_truth.py 固定 IREF 手转实测：A/B=1.016 平衡，A/C=1.72；KCL 违约 0.142；iq@2×e=0.061A 污染电流环）。固件审计排除软件侧（三通道同构无增益项）。首选硬件嫌疑：C 通道运放 Rf 焊错料。
  2. **COG LUT 三连 bug**（此前"错相负优化"的完整机制）：
     - **帧错位**：cogging_calibrate.py 按 theta_user（N 帧 p[3]，bin0 在 0°）分 bin，固件按 θ_mech+φ 且 bin0 在 −180° 索引（foc_app.c:1181）→ 对齐条件 φ\*=(180−z) mod 360，从未被设对（默认 60° 是拍的）。
     - **Flash 遮蔽**：内部电压拖拽辨识（CAL:ALL）的 LUT 存 Flash 并在启动时覆盖编译版（foc_app.c:2554）。其幅值是拖拽电流标尺（非前馈电流标尺）→ 实测注入 22 阶 ≈0.068A vs 编译版 0.0089A（**7× 过驱动**）。v3 扫描"全相位 th_pp 21-31° 爆表、高速恶化 8-9×"全是它的签名。矢量一致性检验（min/max 反推）是定位工具。
     - **锯齿波形**：逐 bin 标定噪声≈信号（pp 0.040A vs 22 阶 0.0089A，~6.8 样本/bin），相邻 bin 阶跃 0.021A，任意相位都是扰动源。

### Resolution
1. **CH_CFG 运行时补救**（adc_sampling.c）：`CMD:CH_CFG,gain_c,recon` 增益垫/两相重构（ic=−(ia+ib)），RAM-only。重构后低速纹波塌 8-18×（E3 0.5°/s std 246→32），KCL 0.142→0.002，C 因果链终审通过。
2. **谐波重构平滑 LUT**（cog_lut_rebuild.py）：rfft 只留 22/44 阶，幅值相位严格保留，pp 0.040→0.0226A，已写入 cogging_lut_cal.h。
3. **相位扫描工具**（cog_phase_sweep.py v2）：PREF 锚点接力（v1 段间阶跃回拉伪影修复）+ 角度域 LSQ 指标（对 PDB iq_cmd 拟合 sin/cos(22θ+44θ)，速度波动/帧偏置免疫）+ 卡死守卫。齿槽 22/rev → ripple(φ) 周期 16.36°，单周期扫描覆盖全等价类。
4. **固件**：`CMD:COG_LUT,USE_COMPILED`（换回编译版，回显 min/max 对拍）/ `CMD:COG_LUT,SAVE`（停电机后固化 Flash）；Flash 加载失败不再误废编译版（旧 else 分支 valid=0 静默陷阱修复）；Makefile 补 foc_observer.c（GCC 全链接此前必炸的存量缺口）。

### Verification（台架实测，编译版平滑 LUT + phi\*=0°）
| 场景 | OFF | ON | 判定 |
|---|---|---|---|
| 5°/s 扫描最优相位 | amp22 0.0101A | **0.0014A** | 对消点清晰（其余相位 0.007-0.019） |
| 10/30/50°/s amp22 | 0.0087-0.0094 | **0.0007-0.0009** | **对消 90-92%** |
| 全速档 th_pp | 2.2-3.8° | **1.7-2.3°** | 改善 |
| OFF 基线自检 | 0.0101 vs 标定 0.00892 | — | 测量链一致 ✓ 漂移 1% ✓ |

- phi\*=0° ⇒ z≈180°，与 φ\*=(180−z) mod 360 帧分析自洽闭环。
- 已执行 `CMD:COG_LUT,SAVE`（attempted=1），Flash 内容=编译版，遮蔽无害化。

### Prevention / Follow-up
- **C 相硬件排查**（用户侧）：C 通道运放 Rf/Rin 实值 → 分流电阻 → 接插件；修好后 CH_CFG 复归默认并全套复验（0.5°/s 定版参数是在坏 C 下调的）。
- **COG 运行时默认**：重启后 gain=0/phase=+60°（Init 默认），启用须 `COG_CFG,1.0,0.0`；是否翻转默认待定版讨论。
- cogging_calibrate.py v2：θ_mech 帧分 bin（读 z 换算）+ 多样本降噪，消除对运行时 φ 旋钮的依赖。
- 挂账：E2/E3 矩阵重跑看 run 间方差；位置环提频裁定仍挂起。

### Commits
- （工作区，待提交：CH_CFG + 平滑 LUT 头文件 + COG_LUT 命令 + Makefile + 三件 scripts 工具）
