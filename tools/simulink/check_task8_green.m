...
assert(positionLoopPorts(1) == 5 && positionLoopPorts(2) == 1, ...
    'Task 8 position loop ports are incorrect.');
assert(speedLoopPorts(1) == 4 && speedLoopPorts(2) == 2, ...
...
assert(~isempty(find_system(modelName, 'SearchDepth', 1, 'Name', 'Kd_pos')), ...
    'Task 8 top level is missing Kd_pos.');
...
assert(strcmp(get_line_source_parent(phPos.Inport(3)), [modelName '/speed loop']), ...
    'Position loop speed feedback is not fed by the speed loop.');
assert(get_line_source_port_index(phPos.Inport(3)) == 2, ...
    'Position loop is not using the observed speed output from speed loop port 2.');
assert(strcmp(get_line_source_parent(phPos.Inport(4)), [modelName '/Kp_pos']), ...
    'Position loop Kp is not fed by Kp_pos.');
assert(strcmp(get_line_source_parent(phPos.Inport(5)), [modelName '/Kd_pos']), ...
    'Position loop Kd is not fed by Kd_pos.');
...
assert(~isempty(find_system([modelName '/position loop'], 'SearchDepth', 1, 'Name', 'wrap_pos_error')), ...
    'Position loop does not wrap angle error to the shortest path.');
assert(~isempty(find_system([modelName '/position loop'], 'SearchDepth', 1, 'Name', 'd_term')), ...
    'Position loop does not contain the PD damping term.');
assert(~isempty(find_system([modelName '/speed loop'], 'SearchDepth', 1, 'Name', 'theta_prev')), ...
    'Speed loop is missing the previous-angle state.');
assert(~isempty(find_system([modelName '/speed loop'], 'SearchDepth', 1, 'Name', 'wrap_delta_theta')), ...
    'Speed loop does not wrap delta_theta to [-pi, pi].');
assert(~isempty(find_system([modelName '/speed loop'], 'SearchDepth', 1, 'Name', 'speed_prev')), ...
    'Speed loop is missing the filtered speed state.');
assert(~isempty(find_system([modelName '/speed loop'], 'SearchDepth', 1, 'Name', 'speed_mech_fb')), ...
    'Speed loop does not expose the observed speed output.');
assert(strcmp(get_line_source_parent(phSpeed.Inport(2)), [modelName '/feedback']) && ...
    get_line_source_port_index(phSpeed.Inport(2)) == 3, ...
    'Speed loop input 2 is not fed by feedback/theta_mech_fb.');
...
function idx = get_line_source_port_index(inportHandle)
lineHandle = get_param(inportHandle, 'Line');
srcPort = get_param(lineHandle, 'SrcPortHandle');
srcParent = get_param(srcPort, 'Parent');
portHandles = get_param(srcParent, 'PortHandles');
idx = find(portHandles.Outport == srcPort, 1, 'first');
end
